...
def binpack(capacity: int, weights: list) -> Union[None, list[list[int]]]:
    ...
    x = dict(((i, j), solver.IntVar(0, 1, f'x_{i}_{j}')) for i in range(n_widgets) for j in range(n_widgets))
    ...
    for j in range(n_bins):
        solver.Add(sum(x[i, j] * weights[i] for i in range(n_widgets)) <= y[j] * capacity)
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        for j in range(n_bins):
            if y[j].solution_value() == 1:
                bin_item_weights.append(sorted((weights[i] for i in range(n_widgets) if x[i, j].solution_value() > 0), reverse=True))
